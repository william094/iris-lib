...
func (e *executor) runTask(writer http.ResponseWriter, request *http.Request) {
	...
	e.log.Info("任务参数:%v", zap.Any("param", param))
...
func (e *executor) taskLog(writer http.ResponseWriter, request *http.Request) {
	...
	req := &LogReq{}
	if err != nil {
		e.log.Error("日志请求失败:", zap.Error(err))
	...
	e.log.Info("日志请求参数:%+v", zap.Any("req", req))
	if e.logHandler != nil {
...
func (e *executor) idleBeat(writer http.ResponseWriter, request *http.Request) {
	...
	e.log.Info("忙碌检测任务参数:%v", zap.Any("param", param))
	_, _ = writer.Write(returnGeneral())
}
...
func (e *executor) callback(task *Task, code int64, msg string) {
	...
	if err != nil {
		e.log.Error("callback err : ", zap.Error(err))
	}
	body, err := ioutil.ReadAll(res.Body)
	if err != nil {
		e.log.Error("callback ReadAll err : ", zap.Error(err))
	}
...
// TaskBeat 任务日志
func (e *executor) TaskBeat(writer http.ResponseWriter, request *http.Request) {
